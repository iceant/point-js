;(function(window, document){
    ...
    for (var i = 0, type; type = ['String', 'Function', 'Array', 'Number', 'Boolean', 'Object'][i++];) {
        (function (type) {
            pointjs.prototype['is'+type] = Type['is' + type] = function(obj){return _isObjTypeFn(obj, type)};
        }(type));
    }
    ...
    pointjs.prototype.loadCss=function( url ){
        return new Promise(function(resolve, reject) {
            var link = document.createElement("link");
            link.type = "text/css";
            link.rel = "stylesheet";
            link.href = url;
            EventUtil.on(link, 'load', resolve);
            EventUtil.on(link, 'error', reject);
            document.getElementsByTagName("head")[0].appendChild(link);
        });
    };

    pointjs.prototype.unloadCss=function(url){
    ...
    pointjs.prototype.loadScript=function( url ){
        return new Promise(function(resolve, reject){
            var script = document.createElement( "script" );
            script.type = "type/javascipt";
            script.src = url;
            EventUtil.on(script, 'load', resolve);
            EventUtil.on(script, 'error', reject);
            (document.getElementsByTagName('head')[0] || document.body || document.documentElement).appendChild(script);
        });

    }

    pointjs.prototype.unloadScript = function(url){
    ...
    pointjs.prototype.sleep = pointjs.prototype.wait =function(time){
        return new Promise(function(resolve, reject){
